        ...
        <div class="flex flex-wrap -mx-3 mb-4">
          <div class="w-full md:w-1/2 px-3 flex-col mb-4">
            ...
            <input
              ...
              v-model="form.vendor"
              :class="{ 'border-red-500': errors.vendor.is, 'border-gray-300': !errors.vendor.is }"
            />

            <template v-if="errors.vendor.is">
              <span v-for="error in errors.vendor.messages" class="border-red-700 block px-2 py-2 text-sm text-red-100 bg-red-500">
                {{ error }}
              </span>
            </template>
          </div>
          ...
          <div class="w-full md:w-1/2 px-3 flex-col mb-4">
            ...
            <input
              ...
              :class="{ 'border-red-500': errors.package.is, 'border-gray-300': !errors.package.is }"
            />

            <template v-if="errors.package.is">
              <span v-for="error in errors.package.messages" class="border-red-700 block px-2 py-2 text-sm text-red-100 bg-red-500">
                {{ error }}
              </span>
            </template>
          </div>
...
export default {
  ...
  methods: {
    submit () {
      this.$inertia.post(this.$route('site_types.admin.store'), this.form)
    }
  }
}
</script>
